...
Page({
	...
	onLoad: async function (options) {
		...
		wx.setNavigationBarTitle({
			title: projectSetting.PRODUCT_NAME + '-修改贴子',
		});
	...
	_loadDetail: async function () {
		...
		if (!this.data.isLoad) this.setData(await AdminProductBiz.initFormData(id)); // 初始化表单数据
	...
	bindFormSubmit: async function () {
		...
		try {
			...
			let callback = async () => {
				...
				let node = { 
					'PRODUCT_OBJ': {
						'star': dataHelper.getDataByKey(forms, 'mark', 'star').val, 
					},
					'PRODUCT_TITLE': data.title,
					'PRODUCT_CATE_NAME': data.cateName.join(' - '),
